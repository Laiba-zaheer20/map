...
class Admin extends CI_Controller {
    ...
    function order_item() {
      ...
      foreach ($data as $key => $value) {
        ...
        if ($orderfields['qty']<$orderdetails[0]['good_qty']) {
        $data =  array(
          ...
          $more_arr=[];
            $more_arr[0]=[
                ...
                'size_in_inches' => $orderdetails[0]['size_in_inches'],
    ...
    function create_invoice( $order_id='') {
      ...
      redirect('admin/print_orderdetails/'.$invoice_id);
    ...
    //   }
    //
    //   $order_details = $this->admin_model->getInvoiceDetails($invoice_id);
    //
    //   $data=array(
    //
    //     'title'=>"New Invoice",
    ...
    //
    //     'order' => $order_details
    //     // "order_more" => $this->basic_model->getRows2('orders_more','order_id',$order_details['order_id']),
        ...
        if($id == ''){ // insert
            ...
            for($i=0;$i<$count;$i++)
            {
              if(!empty($_FILES['image']['name'][$i]))
              {
                ...
                // $a= rand().$file_ext;
                $img_name=rand(1000,9999).$_FILES['image']['name'][$i];
    ...
    public function getOrders($status = 'incoming') {
      ...
      foreach($result as $row)
      {
        ...
        if ($this->session->userdata('is_customer')=='yes') {
         ...
         if ($this->session->userdata('is_img')=='yes') {
           if(file_exists('uploads/'.$row['image'])) {
             $sub_array[] = "<a href='".base_url('uploads/'.$row['image'])."' target='_blank'><img src='".base_url('uploads/'.$row['image'])."' style='height:50px!important; min-width: 10px; max-width: 50px; max-height: 50px' class='img-thumbnail' /></a>";
           } else {
             $sub_array[] = "No Image";
           }
         }
         ...
         if ($this->session->userdata('is_status')=='yes') {
          ...
          if($this->session->userdata('role') == "admin") {
            $sub_array[] = "<a href='javascript:void(0)' onclick='show_modal(".$row['id'].")'><span class='badge badge-".$badge_color."'>".$row['status']."</span></a>";
          } elseif ($this->session->userdata('role') == "customer") {
             $sub_array[] = "<a href='javascript:void(0)'><span class='badge badge-".$badge_color."'>".$row['status']."</span></a>";
          }
         }
       ...
       $buttons .= "<a href='".site_url('admin/reorder/'.@$row['id'].'/'.$row['status'])."' class='btn small-btn reorderproduct' ><i class='ft-rotate-ccw'></i> Reorder </a>";
        ...
        if ($this->session->userdata('is_status')=='yes') {
       if($this->session->userdata('role') == "admin") {
         $buttons .= " <a href='javascript:void(0)' class='btn small-btn' onclick='show_modal(".$row['id'].")'><i class='icon-arrow-left'></i> Change status </a>";
       }
      }
    ...
    public function change_status() {
      if($this->input->post()) {
        ...
        if ($status == 'received') {
          ...
          $this->basic_model->updateRecord(array('status' => $status,'received_date' => $this->input->post('received_date')), 'orders', 'id', $id);
    ...
    public function dashboard()
    {
      ...
      if ($this->session->userdata('role') == "admin") {
        ...
        $data = array(
          ...
          'pastdue_orders' => $this->basic_model->get_pastdueorders_graphs('new'),
        ...
        $data = array(
          ...
          'shipped' => $this->basic_model->get_graphs('incoming',$_SESSION['user_id'],$start_data,$end_data),
          'inventories' => $this->basic_model->get_inv_graphs('incoming',$_SESSION['user_id']),
          ...
          'pastdue_orders' => $this->basic_model->get_pastdueorders_graphs('new',$_SESSION['user_id']),
        );
        ...
        for($row=2; $row<=$highestRow; $row++)
        {
           ...
           $upc = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
            ...
            $order_data = array(
              ...
              'upc' => $upc,
              'part_no' => $part_no,
              'image' => '76262.png2wbmp(pngname, wbmpname, dest_height, dest_width, threshold)',
              'supplier' => $supplier,
    ...
    function archives_itemordered($status="")
    {
        ...
        foreach ($this->cart->contents() as $items) {
          ...
          $output .= '
            <tr data-max-qty="'.$items['options']['max_qty'].'">
                ...
                <button type="button" class="btn btn-danger remove_cart" data="'.$items['rowid'].'" >Reomve</button>
                </td>
            </tr>
          ';
        }
