       ...
       Form6(void)
		{
			...
			while (edge_count < V - 1) { 
				...
				for (int i = 0; i < V; i++) { 
					for (int j = 0; j < V; j++) { 
						if (f1->find(i,parent) != f1->find(j,parent) && cost[i][j] < min) { 
							min = cost[i][j]; 
							a = i; 
							b = j; 
						} 
			    ...
			    int x = f1->find(a,parent); 
				int y = f1->find(b,parent); 
				parent[x] = y; 
 
					this->listView1->Items->Add("Edge"+ (edge_count+1) + " : ("+ a +", "+b+")"+ "cost: "+min);
		//			printf("Edge %d:(%d, %d) cost:%d \n", 
		//				edge_count++, a, b, min); 
					mincost += min; 
				} 
					this->listView1->Items->Add("Minimum Cost : "+ mincost);
		} 
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button1->Size = System::Drawing::Size(540, 61);
			...
			// listView1
			// 
			this->listView1->Location = System::Drawing::Point(35, 122);
			this->listView1->Name = L"listView1";
			this->listView1->Size = System::Drawing::Size(189, 271);
			this->listView1->TabIndex = 6;
			this->listView1->UseCompatibleStateImageBehavior = false;
			this->listView1->SelectedIndexChanged += gcnew System::EventHandler(this, &Form6::listView1_SelectedIndexChanged);
			// 
			// button2
			// 
			this->button2->Location = System::Drawing::Point(272, 143);
			...
			this->ClientSize = System::Drawing::Size(618, 450);
